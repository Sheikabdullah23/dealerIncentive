...
import jcbLogo from '@salesforce/resourceUrl/JCB_Logo';
...
export default class JCBIncentiveCalculator extends LightningElement {
    ...
    @track kpiSize = "slds-max-small-size--1-of-1 slds-medium-size--12-of-12 slds-large-size--6-of-12 subProduct_flex mobile-view";
    @track kpiClass = 'slds-grid slds-wrap slds-size_1-of-1 slds-medium-size_3-of-6 slds-large-size_6-of-12 kpi-container';
    @track summaryClass = 'slds-grid slds-wrap slds-size_1-of-1 slds-medium-size_3-of-6 slds-large-size_6-of-12 slds-p-horizontal_x-small';
    @track incRecord = {};
    ...
    currentTab;
    @track initialMessage = "Select the Year and Month fields above, then click the 'Check Incentive' button to load the respective incentive configuration.";
    ...
    getProductKpiPayoutInfo(product) {
        ...
        for (let i = 0; i < kpIs?.length; i++) {
            ...
            if ((kpiTar || addKpiTar) && kpIs[i].KPI__r.KPI_Visiblity__c.includes('KPI Payout')) {
    ...
    getProductKpiAchivementInputs(product) {
        ...
        for (let i = 0; i < kpIs?.length; i++) {
            ...
            if (kpiTar || addKpiTar || growthKpiTar || multiKpiTar) {
                ...
                if (tivTarget.slabInfo?.length > 0) {
                            ...
                            if (slabs[s].slabType == 'Total Volume Industry Incentive') {
                                ...
                                if (!isCalcOrTarget) {
                                    if (validateTargetConditon(slabs[s].slabRange, coverageActual)) {
                                        product.tivPercentageActual = slabs[s].payoutPercentage ? slabs[s].payoutPercentage : 0;
                                    }
                                }
                            }
                        }
                    ...
                    if (!isEligible) {
                        if (isCalcOrTarget) {
                            ...
                            product.tivPercentageCalc = 0;
                        } else {
                            product.totalTIVTargetPayout = 0;
                            product.tivPercentageActual = 0;
                        }
                    }
                } else {
    ...
    downloadReport() {
        ...
        getPdfcontent({ month: this.month, year: this.year, dealerId: this.accountDet.Id })
            .then((res) => {
                ...
                this.isLoading = false;
                this.showToast('Dealer Incentive Report has been downloaded successfully!', 'success');
            })
            .catch((e) => {
                this.isLoading = false;
            })
    ...
    selectAdditionalProduct() {
        ...
        for (let [key, value] of this.productsAndPayout) {
            ...
            product.isVisible = this.addkpiTargetInfo.get(key) || this.growthKPITargetInfo.get(key) ? true : false;
    ...
    handleShowCost(event) {
        ...
        this.kpiSize = checkbox.checked ? "slds-max-small-size--1-of-1 slds-medium-size--12-of-12 slds-large-size--4-of-12 subProduct_flex mobile-view" : "slds-max-small-size--1-of-1 slds-medium-size--12-of-12 slds-large-size--6-of-12 subProduct_flex mobile-view";
        this.summaryClass = checkbox.checked ? "slds-grid slds-wrap slds-size_1-of-1 slds-medium-size_3-of-6 slds-large-size_12-of-12 slds-p-horizontal_x-small" : "slds-grid slds-wrap slds-size_1-of-1 slds-medium-size_12-of-12 slds-large-size_6-of-12 slds-p-horizontal_x-small";
    }
    ...
    handleRowClick(event) {
        this.handleMenuSelection(event);
    }

    @track isModalOpen = false;

    handleMenuClic(event) {
    ...
    get modalClasses() {
        return `modal-content ${this.isModalOpen ? 'open' : ''}`;
    }

    handleMenuSelectionAndCloseModal(event) {
        this.handleMenuSelection(event);
        this.closeModal();
    }

    handleAddSelectionAndCloseModal(event) {
        this.handleAddSelection(event);
        this.closeModal();
    }
    ...
    @track currentStep = 1; // Default to step 1
    @track isWalkthroughVisible = true; // Walkthrough visibility
    @track steps = [
        { content: ' Configure Year for which you are going to calculate incentive.' },
        { content: ' Configure Month for which you are going to calculate incentive.' },
        { content: ' Use these buttons to toggle between projection and actual mode.' },
        { content: ' Use Check Incentive button to view KPI-wise incentive.' },
        { content: ' Total Projected Incentive tab shows the incentive calculated in the calculator and it may change with respect projection/actual mode.' },
        { content: ' Total target Incentive shows the target that is being configured and it may change with respect projection/actual mode.' },
        { content: 'This Tab will show incentive information KPI-wise and Check Details button will show incentive information product wise.'},
        { content: ' Sales Tab will show Incentive amount based on your selection of the product and you can select the products and you can also select additional incentive tab.' },
        { content: ' In the projection mode you can calculate the incentive amount based on the input you are entering in the above input field.'},
        { content: ' For value you have entered in the input field can be calculated and reflected here in Retail Section.'},
        { content: ' Touch this tab anywhere to flip the tab and follow the link below to reach the help page.'}
    ...
    // Dynamic visibility controls for positioning
    get isStep1() {
    ...
    handleStepChange(event) {
        this.currentStep = event.detail; // Update the current step dynamically
    }

    handleWalkthroughComplete() {
        this.isWalkthroughVisible = false; // End the walkthrough
        console.log('Walkthrough complete!');
    }
    get dynamicClass() {
        return `popup-position-step${this.currentStep}`;
    }
